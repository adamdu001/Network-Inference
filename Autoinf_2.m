% compare RG and SF
function f = Autoinf_2(A,N,T,n,omega,y0,K,samples,threshold)

    tspan = linspace(0, T, n);
    dt = T/(n-1);
    y_cel = cell(samples,1);
    for i = 1:samples
        [t,y] = ode45(@(t,theta)ode(t,theta,omega,K(i),N,A),tspan,y0(:,i));
        y_cel{i} = y;
    end
    ...
    for a = 1:samples
        for b = 1:n-1
            for i = 1:N % for each row
                for j = 1:N
                    G{a,b}(i,j) = (K(a)/N)*sin(y_cel{a}(b,j)-y_cel{a}(b,i));
                end
            end
        end
    end
